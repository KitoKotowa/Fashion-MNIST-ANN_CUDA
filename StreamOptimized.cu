    ...
    void train(const vector<vector<float>>& training_data, const vector<int>& labels, int epochs = 10) {
        ...
        for (int epoch = 0; epoch < epochs; ++epoch) {
            ...
            for (size_t i = 0; i < training_data.size(); ++i) {
                ...
                // Copy input data asynchronously using stream1
                CHECK(cudaMemcpyAsync(d_input, training_data[i].data(), input_size * sizeof(float), cudaMemcpyHostToDevice, stream1));

                vector<float> h_target(output_size, 0.0f);
                h_target[labels[i]] = 1.0f;
                // Copy target data asynchronously using stream2
                CHECK(cudaMemcpyAsync(d_target, h_target.data(), output_size * sizeof(float), cudaMemcpyHostToDevice, stream2));
                ...
                backward_propagation_kernel<<<gridDim3, blockDim, shared_mem_size, stream4>>>(
                    d_input, d_target, d_W1, d_b1, d_W2, d_b2, d_W3, d_b3, d_output, d_a1, d_a2, learning_rate,
                    input_size, hidden1_size, hidden2_size, output_size);

                // Synchronize streams
                CHECK(cudaStreamSynchronize(stream3));
                CHECK(cudaStreamSynchronize(stream4));

                // Copy output data back to host asynchronously using stream1
                vector<float> h_output(output_size);
                CHECK(cudaMemcpyAsync(h_output.data(), d_output, output_size * sizeof(float), cudaMemcpyDeviceToHost, stream1));
