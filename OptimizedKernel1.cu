...
__global__ void backward_propagation_atomic_kernel(
    ...
    int input_size, int hidden_size1, int hidden_size2, int output_size) 
{
    ...
    // Backpropagate error to second hidden layer using atomic operations
    if (idx < hidden_size2) {
        float sum = 0.0f;
        for (int i = 0; i < output_size; i++) {
            sum += W3[i * hidden_size2 + idx] * delta_output[i];
        }
        delta_hidden2[idx] = (hidden2[idx] > 0.0f) ? sum : 0.0f;
    }
    __syncthreads();

    // Backpropagate error to first hidden layer using atomic operations
    if (idx < hidden_size1) {
        float sum = 0.0f;
        for (int i = 0; i < hidden_size2; i++) {
            sum += W2[i * hidden_size1 + idx] * delta_hidden2[i];
        }
        delta_hidden1[idx] = (hidden1[idx] > 0.0f) ? sum : 0.0f;
    }
    __syncthreads();
    ...
    if (idx < output_size) {
        for (int i = 0; i < hidden_size2; i++) {
            float update = -learning_rate * delta_output[idx] * hidden2[i];
            atomicAdd(&W3[idx * hidden_size2 + i], update);
        }
        atomicAdd(&b3[idx], -learning_rate * delta_output[idx]);
    }

    if (idx < hidden_size2) {
        for (int i = 0; i < hidden_size1; i++) {
            float update = -learning_rate * delta_hidden2[idx] * hidden1[i];
            atomicAdd(&W2[idx * hidden_size1 + i], update);
        }
        atomicAdd(&b2[idx], -learning_rate * delta_hidden2[idx]);
    }

    if (idx < hidden_size1) {
        for (int i = 0; i < input_size; i++) {
            float update = -learning_rate * delta_hidden1[idx] * input[i];
            atomicAdd(&W1[idx * input_size + i], update);
        }
        atomicAdd(&b1[idx], -learning_rate * delta_hidden1[idx]);
    }
}
    ...
    void train(const vector<vector<float>>& training_data, const vector<int>& labels, int epochs = 10) {
        for (int epoch = 0; epoch < epochs; ++epoch) {
            ...
            for (size_t i = 0; i < training_data.size(); ++i) {
                ...
                dim3 gridDim((max(max(hidden1_size, hidden2_size), output_size) + BLOCK_SIZE - 1) / BLOCK_SIZE);    
                ...
                backward_propagation_atomic_kernel<<<gridDim, blockDim, shared_mem_size>>>(
                    d_input, d_target, d_W1, d_b1, d_W2, d_b2, d_W3, d_b3,
                    d_output, d_a1, d_a2, learning_rate,
